...
To run tests in docker `docker run -it cypress` it will run command `npm run test` - this is command from package.json. In case you want to use different command modify in Dockerfile `CMD ["npm", "run", "test"]` section.  
